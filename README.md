<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="strategy-prompts — 전략·마케팅·기획·PR·리서치·보고서 프레임워크 63개, ChatGPT·Claude·Gemini용 한국어 해요체 프롬프트">
</p>

<p align="center">
  <strong>v2.5.0</strong> · 한국어 해요체 · 프롬프트 <strong>63개</strong> (본 42 + 부록 21) · 2026년 8월 기준
</p>
...
Part 6를 **데이터·분석·리서치·보고서**로 확장했습니다. 전략 리서치부터 슬라이드 인포그래픽까지 한 체인으로 이어집니다.

| ID | 추가 내용 |
|----|----------|
| **6-4** | 전략 리서치 설계·실행 (KEQ, 데이터맵, 워크플랜) |
| **6-5** | 데이터 분석·마이닝 (EDA, 세그먼트, So What) |
| **6-6** | 데이터 기반 전략 보고서 (SCQA, 옵션·권고) |
| **6-7** | 전략 슬라이드 덱·템플릿·와이어프레임 |
| **6-8** | 슬라이드 디자인 가이드 (색·타이포·컴포넌트) |
| **6-9** | 인포그래픽·도식화 구현 스펙 |

**새 체인**: `6-4 → 6-5 → 8-18 → 6-6 → 6-7 → 6-8 → 6-9` · 상세: [CHANGELOG.md](CHANGELOG.md) · [workflows/WORKFLOWS.md](workflows/WORKFLOWS.md)
...
현장에서는 MECE로 문제를 쪼개고, 데이터로 가설을 검증하고, 보고서·슬라이드까지 같은 논리로 이어 갑니다. 이 레포는 그 틀을 매번 새로 짜느라 쓰는 시간을 줄이기 위해, 자주 쓰는 프레임워크를 **복사 가능한 프롬프트**로 박아 둔 라이브러리예요.

| 잘 썼을 때 | 대충 썼을 때 |
|-----------|-------------|
| 표·우선순위·슬라이드 골격이 잡힌 초안을 바로 팀에 넘길 수 있어요 | "좀 더 구체적으로요" 피드백만 쌓이고 결국 사람이 다시 씁니다 |
...
<p align="center">
  <img src="./assets/readme/section-parts.svg" width="100%" alt="8개 도메인 Part — 전략, 마케팅, 커머스, 기획, PR, 데이터·리서치·보고서, 조직, 부록">
</p>
...
| 모델 | API ID | 파일 | 잘 맞는 일 |
|------|--------|------|-----------|
| **ChatGPT** | `gpt-5.6-sol` (Sol / Terra / Luna) | [chatgpt/prompts.md](chatgpt/prompts.md) | 표·리포트·덱 골격, outcome-first, Think 슬라이더 |
| **Claude** | `claude-opus-5` (Sonnet 5 병행) | [claude/prompts.md](claude/prompts.md) | 긴 맥락·리서치·마이닝, 위기·전략 합성, 1M 컨텍스트 |
...
| 6 데이터·리서치 | Sol · 6-6·6-7 덱 | Opus 5 · 6-4·6-5·6-8·6-9 | 6-1·6-5 대용량 EDA |
...
| Part 6 | 9개 — GA4·실험부터 리서치·보고서·슬라이드까지 |
| 문체 | 한국어 해요체 (`~해요`, `~해 주세요`) |
